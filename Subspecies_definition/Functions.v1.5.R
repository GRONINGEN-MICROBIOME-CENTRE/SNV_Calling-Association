Find_pheasable_samples = function(AF, Samples, Matrix, homozygous_threshold = 0.9, percentage_homozygous = 0.8){
  ...
  Samples[,Homozygous_perc := Heterogeneity_90 ] -> Samples
  colnames(Samples) = c("ID","Cohort","Homozygous_perc" )
  
  cbind(Matrix, Samples)[ Homozygous_perc > homozygous_threshold] -> Matrix_clustering
  cbind(AF_clean, Samples)[ Homozygous_perc > homozygous_threshold]  -> AF_clustering
  
  Matrix_clustering[ ,c("ID","Cohort","Homozygous_perc" )] -> Samples_clustering
  Matrix_clustering[,!c("ID","Cohort","Homozygous_perc" )]  -> Matrix_clustering
  AF_clustering[,-c("ID","Cohort","Homozygous_perc" )]  -> AF_clustering
...
Remove_SNPLocations_with_high_missingRate = function(Matrix, SNP, Porcentage_missing=0.6){
  ...
  Keep_SNP = colnames(Matrix)[SNP_missingRate_cluster < Porcentage_missing]
  Matrix[,..Keep_SNP] -> Matrix_clustering
  SNP[X2 %chin% Keep_SNP ,] -> SNP_filterd
...
Remove_rare_SNP = function(Matrix, SNP, MAF_filter = 0.1){
  ...
  apply(Matrix, 2, function(x){ x = x[!is.na(x)]
  AFR = ( 2*sum(x==0) +  sum(x==1) ) /(2*length(x))
  if(AFR > 0.5){ return(1-AFR) }else{ return(AFR) } } ) -> MAF_n
  Keep_SNP = colnames(Matrix)[MAF_n>MAF_filter]
  Matrix[, ..Keep_SNP ] -> Matrix_clustering
  SNP[X2 %chin% Keep_SNP,] -> SNP_clustering
  return(list(Matrix_clustering, SNP_clustering))
}  

Perform_clustering = function(Distance, PC,  minClusterSize = 5, psCut=0.8, threads=1, Iterations=10){
  ...
  if(nSamples >= 10){ # need at least 10 samples to do this
    ...
    clusNumStabilityIter <- Iterations # increases time a lot so 10 is enough
